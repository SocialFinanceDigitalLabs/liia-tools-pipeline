...
def create_previous_and_next_episode(
    ...
    for column in columns:
        previous_values = dataframe[column].shift(1)
        next_values = dataframe[column].shift(-1)

        dataframe[column + "_previous"] = previous_values.mask(
            dataframe["CHILD"] != dataframe["CHILD"].shift(1)
        )
        dataframe[column + "_next"] = next_values.mask(
            dataframe["CHILD"] != dataframe["CHILD"].shift(-1)
        )
...
def _update_dec_stage1(row: pd.Series) -> date:
    """
    ...
    """
    end_of_year = date(row["YEAR"], 3, 31)
    if row["Has_open_episode_error"]:
...
def stage_1(ssda903_df: pd.DataFrame, columns: list) -> pd.DataFrame:
    ...
    ssda903_df_stage1_applied = apply_stage1_rules(ssda903_df_stage1)
