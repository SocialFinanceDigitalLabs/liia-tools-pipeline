...
def load_pipeline_config():
    ...
    try:
        with importlib.resources.open_text(
            f"{region_config}_pipeline_config", "placements_standard_pipeline.json"
        ) as f:
        ...
        logger.info(
            f"Configuration file 'placements_standard_pipeline.json' not found in '{region_config}'."
        )
...
def load_schema(year: int, month: str, schema_version: str) -> DataSchema:
    ...
    for fn in all_schema_files:
        ...
        schema_lookup.append((fn, int(match.group(1)), month_map[match.group(2)], match.group(3), match.group(4) is not None))
    ...
    # Filter only those that match the regional schema version
    schema_lookup = [x for x in schema_lookup if x[3] == schema_version]

    # If we have no schema files, raise an error
    if not schema_lookup:
        raise ValueError(f"No schema files found for year {year}, month {month} and schema_version {schema_version}")

    # Find the latest complete schema
    last_complete_schema = [x for x in schema_lookup if not x[4]][-1]
