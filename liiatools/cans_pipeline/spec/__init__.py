...
def load_pipeline_config():
    ...
    try:
        with importlib.resources.open_text(
            f"{region_config}_pipeline_config", "cans_pipeline.json"
        ) as f:
        ...
        logger.info(
            f"Configuration file 'cans_pipeline.json' not found in '{region_config}'."
        )
...
def load_schema() -> DataSchema:
    ...
    """
    schema_path = Path(SCHEMA_DIR, "CANS_schema.yml")
...
def load_summary_sheet_mapping() -> dict:
    ...
    """
    summary_shet_mapping_path = Path(SCHEMA_DIR, "summary_sheet_mapping.yml")

    with open(summary_shet_mapping_path, "r", encoding="utf-8") as f:
        mapping = yaml.load(f)
    return mapping
