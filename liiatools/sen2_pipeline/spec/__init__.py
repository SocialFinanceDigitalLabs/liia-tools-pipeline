...
def load_pipeline_config():
    try:
        with importlib.resources.open_text(
            f"{region_config}_pipeline_config", "sen2_pipeline.json"
        ) as f:
        ...
        logger.info(
            f"Configuration file 'sen2_pipeline.json' not found in '{region_config}'."
        )
...
def load_schema(year: int) -> (xmlschema.XMLSchema, Path):
    return (
        xmlschema.XMLSchema(SCHEMA_DIR / f"sen2_schema_{year:04d}.xsd"),
        Path(SCHEMA_DIR, f"sen2_schema_{year:04d}.xsd"),
    )
