# Take from stream_pipeline in cin_census_pipeline folder
import logging
...
# We need to write stream_record in csww folder so this whole code works
# from liiatools.csww import stream_record
...
# Objects are mentioned after : symbol below (some we may have come up with)
def task_cleanfile(
...
"""
    ...
    with src_file.open("rb") as f:
        ...
        # If empty spreadsheets are put into the secure data platform
        dataset = dataset_holder.value
        if not dataset:
            log.info("No dataset created from file")
        errors = error_holder.values
        if not errors:
